# modified album_csv_v1.rb to output to txt filr - output.txt
...
subtotals.each do |year, count|
  ...
  puts "#{year}: #{bar} [#{count}]"
  File.write("output.txt", "#{year}: #{bar} [#{count}]\n", mode: "a") # Append the same string to the output file
end
...
groups.sort_by {|year, rows| year.to_i}.each do |year, rows|
    ...
    rows.each do |row|
      ...
      File.write("output.txt", "#{row["Title"]} - #{row["Band"]} [#{avg_rating}]\n", mode: "a") # Append the same string to the output file
    end
end
...
File.write("output.txt", "\nAverage ranking of all these albums on Best Ever Albums: #{avg_rating}\n", mode: "a") # Append the same string to the output file
